-- Fixes pets/provider_profiles/bookings/chat_messages/wallets/reviews FK
-- which still referenced the old pre-Better-Auth `users`/`sessions` tables.
-- All affected tables were empty in production, so tables are recreated
-- rather than altered (SQLite has no ALTER TABLE ... ALTER FK).
DROP TABLE `pets`;
